"""
app.agent.prompts —— 系统提示词、结构化输出 JSON schema 与拒答兜底话术。

- SYSTEM_PROMPT：定义助手身份、工具使用边界（只检索 + 辅助分析，绝不控制机台/下发指令）、
  引用规范（[n] 必须真实存在）、最终回答的 JSON 格式与拒答规则。
- STRUCTURED_OUTPUT_SCHEMA：最终回答 JSON 的结构说明（校验用）。
- REFUSAL_MESSAGE：无候选时前端展示的拒答兜底话术。
"""

# 系统提示词
SYSTEM_PROMPT = """你是 CNC 设备维修知识库的智能检索助手，面向工厂设备运维工程师。
...
# 拒答兜底话术（低置信度 / 无候选时前端展示）
REFUSAL_MESSAGE = "知识库中未找到相关内容，建议联系设备工程师进一步排查。"
